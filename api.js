...
$(document).ready(function() {
    $('#sub2').click(function() {
        ...
        var AndRegex = /\&/g; // Regular expression for identifying "&"
        var bracketRegex = /\(/g
        ...
        var bracketRemoval = removeComma.replace(bracketRegex,'')
        var andRemoval = bracketRemoval.replace(AndRegex, '');
        ...
        for (var i = 0; i <spanis.length; i++) {
            ...
            if (nextElement == '.') {
                ...
                if(len == 2){
                    ...
                    extractWords[i] = ' (' + extractWords[i][0] + " & " + extractWords[i][1] + ', ' + extractWords[i][2] + ')'
                }
                else if(len>=4){
                    var yearIndex = len - 1
